...
public class CurveSingleSteeringProjectile : MonoBehaviour
{
    ...
    private void SecondPhase()
    {
        if (secondPhaseTimer < secondPhaseTime)
        {
            ...
            if (target)
            {
                ...
                if (target.GetComponent<OnHit>().GetHealth() <= 0)
                {
                    target = null;
                }
            }
    ...
    virtual public Transform GetNearestEnemy(float r)         //当单位受伤死亡后，返回true
    {
        ...
        for(int i = 1 ; i < objs.Length ; i ++)
        {
            ...
            {
                if (objs[i].GetComponent<OnHit>().GetHealth() > 0)
                {
    ...
    public void Explosion()
    {
        ...
        {
            enemyWhoWasShot[i - 1].GetComponent<OnHit>().OnHit(damage , (enemyWhoWasShot[i - 1].transform.position - transform.position).normalized * Random.Range(20f , 40f));
        }

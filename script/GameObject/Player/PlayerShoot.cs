...
{
    AttackDown = 0 , IntervalDown = 1 , RangeDown = 2 , ScatterDown = 3 , NoDebuff = 4 , CantStopShoot = 5 , DoubleGun = 6 , kaminohikari = 7 , tnnd = 8 , FishRocket = 9
}
public class PlayerShoot : MonoBehaviour
{
    ...
    [SerializeField]public AudioClip[] MagellanVoices;
    [SerializeField]public AudioClip[] KamiVoices;
    [SerializeField]public AudioClip[] GavialVoices;
    [SerializeField]public AudioClip fishRocketFireSound;
    [SerializeField]private AudioClip enhance;
    [SerializeField]public ParticleSystem enhanceParticle;
    ...
    private void ShootColdTimeChange()
    {
        shootColdTime *= 2f/3f;
    ...
    {
        if(shootColdTime > 0.1f)
            // ShootColdTimeChange(0.1f);
            ShootColdTimeChange();
        else
        {
            transform.GetComponent<PlayerMove>().RestoreHP(10);
    ...
    {
        SoundManager.Instance.PlayEffectSound(enhance);
        enhanceParticle.Play();
    }
    ...
    private void Shoot()
    {
        if (pause)
        {
            ...
            GameObject bullet = ObjectPool.Instance.GetObject(bulletPrefab , firePoint.transform.position , Quaternion.identity);
    ...
    private void MoveFirePoint()
    {
        if (pause)
        {
            ...
            firePointPosition.x = firePointDirection.normalized.x + transform.position.x;
            firePointPosition.y = firePointDirection.normalized.y + transform.position.y;
            hand.transform.position = firePointPosition;
            hand.transform.right = firePointDirection;
        }
    }

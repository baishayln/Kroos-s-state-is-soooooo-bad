...
public class GavialsSupport : MonoBehaviour
{
    ...
    private void PlayFlyAudio()
    {
        if (rollSoundTimer > 0)
        {
            rollSoundTimer -= Time.deltaTime;
        }
        else
        {
            SoundManager.Instance.PlayEffectSound(rollSound);
            rollSoundTimer = rollSound.length;
        }
    }
    private void PlayHitGroundAudio()
    {
        SoundManager.Instance.PlayEffectSound(hitGroundSound);
    }
    ...
    private void ShootGround()
    {
        ...
        for(int i = enemyWhoWasShot.Length ; i > 0 ; i --)
        {
            ...
            enemyWhoWasShot[i - 1].GetComponent<OnHit>().OnHit(damage , new Vector2(Xspeed , 40f - Xspeed));
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if(isAttack && other.CompareTag("Enemy"))
        {
            ...
            other.GetComponent<OnHit>().OnHit(damage , dir * flySpeed * 1.3f);
        }

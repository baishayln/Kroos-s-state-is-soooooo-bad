...
public class LightingArea : MonoBehaviour
{
    ...
    [SerializeField]public AudioClip[] attackEffects;
    ...
    virtual public Transform GetHighestHealthEnemy(Vector2 lightingAreaSize)
    {
        ...
        highestHealth = objs[0].GetComponent<OnHit>().GetHealth();
        highestHealthEnemy = objs[0].transform;
        for(int i = 1 ; i < objs.Length ; i ++)
        {
            if (objs[i].GetComponent<OnHit>().GetHealth() > highestHealth)
            {
                highestHealth = objs[i].GetComponent<OnHit>().GetHealth();
    ...
    virtual public void LightAttack(Transform target)
    {
        if(target != null)
        {
            ...
            {
                target.GetComponent<OnHit>().OnHit(damage , target.transform.position.x - transform.position.x);      //击退方向应该是上一个目标和当前目标来计算还是自身和当前目标来计算？
                if (nextTarget == null)
                {
                    i = lightingAttackJumpNum;
                }
                target = nextTarget;
                if (target.GetComponent<EnemyBehavior>())
                {
    ...
    private void PlayAudio()
    {
        ...
        //         break;
        // }
        SoundManager.Instance.PlayEffectSound(attackEffects[Random.Range(0 , attackEffects.Length)]);
    }
